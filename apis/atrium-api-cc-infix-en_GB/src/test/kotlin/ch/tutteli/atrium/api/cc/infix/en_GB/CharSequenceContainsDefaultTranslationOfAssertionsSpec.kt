...
class CharSequenceContainsDefaultTranslationOfAssertionsSpec : ch.tutteli.atrium.spec.integration.CharSequenceContainsDefaultTranslationAssertionsSpec(
    ...
    companion object : CharSequenceContainsSpecBase() {
        ...
        private fun getNameContainsDefaultTranslationOf() = "$toContain with search mode defaultTranslationOf was removed from this API"
        ...
        private fun containsAtLeast(plant: Assert<CharSequence>, atLeast: Int, a: Translatable, aX: Array<out Translatable>): Assert<CharSequence> {
            return if (aX.isEmpty()) {
                plant to contain atLeast atLeast value a.getDefault()
            } else {
                plant to contain atLeast atLeast the Values(a.getDefault(), *aX.map { it.getDefault() }.toTypedArray())
            }
        }
        ...
        private fun containsAtMost(plant: Assert<CharSequence>, atMost: Int, a: Translatable, aX: Array<out Translatable>)
            = plant to contain atMost atMost the Values(a.getDefault(), *aX.map { it.getDefault() }.toTypedArray())
        ...
        private fun containsAtMostIgnoringCase(plant: Assert<CharSequence>, atMost: Int, a: Translatable, aX: Array<out Translatable>): Assert<CharSequence> {
            return if (aX.isEmpty()) {
                plant to contain ignoring case atMost atMost value a.getDefault()
            } else {
                plant to contain ignoring case atMost atMost the Values(a.getDefault(), *aX.map { it.getDefault() }.toTypedArray())
            }
        }
    }
}
