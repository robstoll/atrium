...
object CharSequenceContainsSearchersKt {
    ...
    @JvmStatic
    @Deprecated("use the extension fun `value` instead. This fun is only here to retain binary compatibility, will be removed with 1.0.0", ReplaceWith("checkerBuilder.wert(expected)"))
    fun <T : CharSequence> wert(checkerBuilder: CharSequenceContainsCheckerBuilder<T, CharSequenceContainsNoOpSearchBehaviour>, expected: Any): AssertionPlant<T>
        = werte(checkerBuilder, expected)

    @JvmStatic
    @Deprecated("use the extension fun `the` instead. This fun is only here to retain binary compatibility, will be removed with 1.0.0", ReplaceWith("checkerBuilder.werte(expected, *otherExpected)"))
    fun <T : CharSequence> werte(checkerBuilder: CharSequenceContainsCheckerBuilder<T, CharSequenceContainsNoOpSearchBehaviour>, expected: Any, vararg otherExpected: Any): AssertionPlant<T>
    ...
    fun <T : CharSequence> standardUebersetzungVon(checkerBuilder: CharSequenceContainsCheckerBuilder<T, CharSequenceContainsNoOpSearchBehaviour>, expected: Translatable, vararg otherExpected: Translatable): AssertionPlant<T>
        = checkerBuilder.standardUebersetzungVon(expected, *otherExpected)
