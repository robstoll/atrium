 ...
 * @since 0.9.0
 */
inline fun <reified TExpected : Throwable> Expect<out Result<Any?>>.isFailure(): Expect<TExpected> =
 ...
 * it encapsulates an exception of type [TExpected] and that it holds all
 * assertions the given [assertionCreator] creates.
 *
 * @return An assertion container with the new type [TExpected]
 * @throws AssertionError Might throw an [AssertionError] if the assertion made is not correct.
 *
 * @since 0.9.0
 */
inline fun <reified TExpected : Throwable> Expect<out Result<Any?>>.isFailure(
