...
abstract class IterableToHaveElementsAndAnyExpectationsSpec(
    ...
    include(object : AssertionCreatorSpec<Iterable<Double>>(
        ...
        toHaveElementsAndAny.forExpectationCreatorTest("$toBeGreaterThanDescr\\s+: 1.0") { toBeGreaterThan(1.0) }
    ) {})
    include(object : AssertionCreatorSpec<Iterable<Double?>>(
        "$describePrefix[nullable Element] ", oneToSeven().toList().asIterable(),
        toHaveElementsAndAnyNullable.forExpectationCreatorTest("$toBeGreaterThanDescr\\s+: 1.0") { toBeGreaterThan(1.0) }
    ) {})
