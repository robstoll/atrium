...
abstract class IterableNotToHaveElementsOrNoneExpectationsSpec(
    ...
    include(object : AssertionCreatorSpec<Iterable<Double>>(
        ...
        notToHaveElementsOrNone.forExpectationCreatorTest("$toBeGreaterThanDescr\\s+: 10.0") { toBeGreaterThan(10.0) }
    ) {})
    include(object : AssertionCreatorSpec<Iterable<Double?>>(
        "$describePrefix[nullable Element] ", oneToSeven().toList().asIterable(),
        notToHaveElementsOrNoneNullable.forExpectationCreatorTest("$toBeGreaterThanDescr\\s+: 10.0") { toBeGreaterThan(10.0) }
    ) {})
