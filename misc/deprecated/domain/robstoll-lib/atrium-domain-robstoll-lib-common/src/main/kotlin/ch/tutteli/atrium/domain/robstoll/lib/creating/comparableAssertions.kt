...
fun <T1 : Comparable<T2>, T2 : Any?> _isLessThanOrEqual(subjectProvider: SubjectProvider<T1>, expected: T2) =
    ExpectImpl.builder.createDescriptive(subjectProvider, IS_LESS_THAN_OR_EQUALS, expected) { it <= expected }
...
fun <T1 : Comparable<T2>, T2 : Any?> _isGreaterThanOrEqual(subjectProvider: SubjectProvider<T1>, expected: T2) =
    ExpectImpl.builder.createDescriptive(subjectProvider, IS_GREATER_THAN_OR_EQUALS, expected) { it >= expected }
