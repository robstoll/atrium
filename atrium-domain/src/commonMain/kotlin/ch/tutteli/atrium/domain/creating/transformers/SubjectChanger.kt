...
interface SubjectChanger {
     ...
     * @param maybeSubExpectationCreator Optionally, subsequent expectations for the new subject. This is especially
     *   useful if the change fails since we can then already show to you (in error reporting) what you expect about
    ...
    interface FailureHandler<SubjectT, SubjectAfterChangeT> {
         ...
         * the proofs the [maybeExpectationCreator] would have created for the new subject.
         *
...
class FailureHandlerAdapter<SubjectT, SubjectIntermediateT, SubjectAfterChangeT>(
    ...
    override fun createProof(
        ...
        container.changeSubject.unreported(map).let {
            failureHandler.createProof(it._domain, proof, maybeExpectationCreator)
        }
}
