...
import ch.tutteli.atrium.domain.collectForCompositionBasedOnGivenSubject
import ch.tutteli.atrium.domain.collectForFailureHint
import ch.tutteli.atrium.domain.creating.transformers.FeatureExtractor
import ch.tutteli.atrium.domain.creating.transformers.impl.ThrowableThrownFailureHandler.Companion.propertiesOfThrowable
...
class DefaultFeatureExtractor : FeatureExtractor {
    ...
    private fun <FeatureT, SubjectT> appendFailureHint(
        ...
        container.maybeSubject.fold({
            // If the feature extraction fails because the subject is already None and no subExpectations where
            // defined, then we don't need/want to show the fixedClaimGroup because the feature as such will
            // already be shown via explanatory expectation-group at the place where the first
