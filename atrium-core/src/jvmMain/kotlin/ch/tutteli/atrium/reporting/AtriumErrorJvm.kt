    ...
    when (causingProof) {
        is RootProofGroup -> takeIf(causingProof.proofs.size == 1){
            ...
            when (val first = causingProof.proofs.first()) {
                ...
                is  ch.tutteli.atrium.assertions.BasicDescriptiveAssertion -> first.representation
                else -> null
            }
        }
        else -> null
...
fun extractActual(causingProof: Proof): Any? =
    ...
    is RootProofGroup -> takeIf(causingProof.proofs.size == 1) {
        ...
        when (causingProof.proofs.first()) {
            is SimpleProof,
            is ch.tutteli.atrium.assertions.BasicDescriptiveAssertion -> causingProof.representation
            else -> null
        }
    }
    else -> null
